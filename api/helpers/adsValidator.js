...
export function adsValidator(req, res, next) {
  const schema = Joi.object().keys({
    ...
    address: Joi.string().min(3).regex(/\\d+\\s+([a-zA-Z]+|[a-zA-Z]+\\s[a-zA-Z]+)/).required(),
    city: Joi.string().min(3).regex(/([a-zA-Z]+|[a-zA-Z]+\\s[a-zA-Z]+)/).required(),
    state: Joi.string().regex(/([a-zA-Z]+|[a-zA-Z]+\\s[a-zA-Z]+)/).required(),
